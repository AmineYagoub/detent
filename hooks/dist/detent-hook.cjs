...
// src/plugin/hook.ts
var SURFACE_FILE = "active_surface.json";
var STAGE_FILE = "stage.json";
...
function decidePreToolUse(payload) {
  ...
  let raw;
  try {
    raw = (0, import_node_fs.readFileSync)(import_node_path2.default.join(cwd, ".detent", SURFACE_FILE), "utf8");
  } catch {
    return null;
  }
  let cfg;
  try {
    ...
    return denyJson(
      `DENY: ${import_node_path2.default.join(".detent", SURFACE_FILE)} exists but is unreadable \u2014 a declared surface that cannot be honored fails closed (P5).`
    );
  }
...
async function decideStop(payload) {
  const cwd = payloadCwd(payload);
  let stage = "";
  let gateCmd = null;
  try {
    const parsed = JSON.parse((0, import_node_fs.readFileSync)(import_node_path2.default.join(cwd, ".detent", STAGE_FILE), "utf8"));
...
async function handleHookInput(raw) {
  let payload;
  try {
