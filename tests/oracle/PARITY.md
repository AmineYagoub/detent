...
| `test_extra.py::test_three_change_cycles_escalate` | green | T-044 (M2) | `tests/kernel/stages.test.ts` | D-6 divergence: the oracle recycled review changes into a shared fix pool (three cycles to exhaust); review has its own budget here — `review_fix_attempts`, default 3, read from config (X-1‴, PRDR-108) — so the changes verdict after the last round escalates; the preserved property is that repeated changes reach a human |
