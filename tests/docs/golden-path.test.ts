...
describe("T-069 C-5: the interrupt set is frozen at five", () => {
  ...
  it("no module raises a prompt outside the closed set (C-5's lint half)", () => {
    ...
    // presented: the two consent/escalation surfaces and the init CLI. A
    // `readline` anywhere else would be a sixth interrupt class in disguise.
    const SANCTIONED = new Set(["cli/escalate.ts", "cli/verify.ts"]);
