...
acceptance_criteria: ["`contracts.findings` reaches PRESENT. The deterministic checker's findings are the reliable ones and are currently the only ones an operator never sees — the log carries them and the plan output does not.", "The mechanical checker is widened where a thing is EXACTLY decidable. AMENDED on implementation: duplicate file ownership already is and is already checked; ticket-size proxies are NOT — measured, see below.", "Feedback quality is measured directly — finding IDENTITY across rounds, not counts. Whether the revision fixes the findings it was given is unknown today, and no fix can be chosen without it.", "Only then is it decided whether the intrinsic revision round earns its cost, or whether that budget belongs in the deterministic checks."]
...
## Amendment on implementation (2026-09-09): sizing is not mechanically decidable

Criterion 2 asserted that ticket-size proxies "largely are" decidable. They are not, and the
claim was tested rather than assumed. Taking the review's own 16 sizing findings as labels
against the other 252 tickets:

| proxy | oversized | others |
|---|---|---|
| `provides` | 3.7 | 3.0 |
| `acceptance_criteria` | 6.4 | 6.1 |
| `surface` globs | 6.1 | 4.7 |
| "and"s in the title | 0.8 | 0.8 |

`surface` is the strongest and still useless as a gate:

```
surface >= 5: flags 146 tickets, 13 of 16 real -> precision  9%, recall 81%
surface >= 7: flags  41 tickets,  8 of 16 real -> precision 20%, recall 50%
```

**Four in five flags would be wrong at the best threshold available.** A check like that is a
warning that fires constantly, which is V-1‴'s own account of a useless one, so it is not built.
Sizing stays with the reviewer. Recording the measurement is the deliverable here: the next
person to have this idea can see it was tried.

What criterion 2 turned out to need was nothing — the checker ALREADY catches what is decidable.
Run against the finished plan it produces 7 findings including the one certain failure in it
(`t-s12-012` and `t-s14-003` both creating `plugin/skills/init/SKILL.md`). They were computed on
every run and shown to nobody, which is criterion 1, not criterion 2.
