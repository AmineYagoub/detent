...
acceptance_criteria: ["Effort is configurable per ROLE, beside `model_routing`, over the SDK's closed set (`low`|`medium`|`high`|`xhigh`|`max`).", "The default changes nothing: with no effort configured, a session's options are byte-identical to today's. This ticket ships a knob, not a behaviour change.", "An unknown effort level is refused at config load, naming the key — the discipline PRDR-142 established for `model_routing`, which accepted any key so a typo routed a role to the runtime default forever.", "An effort a routed model cannot serve is handled as PRDR-114 handles an unservable model: noted per session, never a silent downgrade the operator cannot see.", "Whatever the config says reaches `buildOptions`. Asserted on the options a session is ACTUALLY built with — SEC-4′ and this line's repeated history are what that sentence is for."]
